...
def mssql_connection(**kwargs):
    mssql_db_conn = get_secret(kwargs["db_conn_secret_name"])
    ...
    try:
        mssql_connection = turbodbc.connect(
            ...
            driver              = mssql_db_conn.get("driver"),
            server              = mssql_db_conn.get("server"),    # "tcp:127.00.000.00,1433" 
            database            = mssql_db_conn.get("database"),
            UID                 = mssql_db_conn.get("user_pass"),
            PWD                 = mssql_db_conn.get("user_pass")
    )
